...
public class SheetBeanConverter extends AbstractConverter implements Converter {
    ...
    public SheetBeanConverter(String fileAbsolutePath) throws ConverterException, FileNotFoundException {
        ...
        this.inputStream = new FileInputStream(fileAbsolutePath);
    }

    public SheetBeanConverter(File file) throws FileNotFoundException, ConverterException {
     ...
     */
    @Override
    public <T> List<T> sheetConvertBean(Class<T> clazz, int startRow) {
        String sheetName = this.getSheetName(clazz);

        try {
            Sheet sheet = this.getWorkSheet(sheetName);
            // Data row size in the work sheet
            ...
            if (rowSize < 2) {
                ...
                Map<String, Field> validFieldMap = this.validFieldsMapScan(clazz);
                // Header info of the work sheet (save at the first line in the work sheet)
                List<String> sheetHeaderList = this.getSheetHeaderInfo(sheet);
                // Get the index list to traverse the specified columns instead of traversing all columns
                List<Integer> validColumnIndex = this.getValidSheetColumnIndex(sheetHeaderList, validFieldMap);
                // Traverse the work sheet rows then generate bean though the row data
                List<T> beanList = this.traverseSheet(sheet, rowSize, startRow, clazz, validColumnIndex, sheetHeaderList, validFieldMap);
    ...
    private Sheet getWorkSheet(String sheetName) throws ConverterException, IOException {
        Workbook workbook = HSSFWorkbookFactory.create(this.inputStream);
        if (workbook == null) {
    ...
    /**
     * Get the table header info of the work sheet,
     * we assume the first line is always save the header info
     */
    private List<String> getSheetHeaderInfo(Sheet sheet) {
     ...
     * Traverse the work sheet rows then generate bean though the row data
     */
    private <T> List<T> traverseSheet(Sheet sheet, int rowSize, int startRow, Class<T> clazz,
        ...
        for (int rowIndex = startRow - 1; rowIndex < rowSize; ++rowIndex) {
